...
def getApiMetadata():
    """
    Reads the AI Proxy token from environment variable and sets the API endpoint.
    """
    api_proxy_token = os.getenv("AIPROXY_TOKEN")
    if not api_proxy_token:
        logging.error("AIPROXY_TOKEN environment variable not found.")
...
def findEncoding(file_path):
    """
    This method uses chardet library to find the encoding for unseen datasets.
    """
    try:
        import chardet
    except ImportError:
        logging.info("chardet library not loaded. Performing pip install...")
...
def identifyClusters(df):
    """
    Use silhouette score to get the optimum clusters.
    """
...
def performPrimitiveOperations(file_path):
    """
    This method is reponsible for reading the dataset and performing primitive analysis, EDA, transformation (imputation, clusting etc.).
    """
    # Detect encoding
    encoding = findEncoding(file_path)
    try:
...
def retryAwareSessionHandlingWithBackoff():
    """
...
def visualizeAndSaveCharts(df, output_dir):
    """
    Generates visualizations based on the DataFrame and saves them as PNG files under passed output directory.
...
def generateInteractiveVisualizations(df, output_dir):
    """
...
def analyzeVisualizations(png_files, api_proxy_token, api_proxy_url):
    """
...
def preparePromptForNarrative(analysis, png_files, interactive_files, api_proxy_token, api_proxy_url):
    """
    Generates a dynamic narrative in Markdown format using the LLM based on the analysis.
    Returns the narrative as a string.
    """
    session = retryAwareSessionHandlingWithBackoff()
...
def narrateStoryWithVisuals(analysis, png_files, interactive_files, api_proxy_token, api_proxy_url):
    """
    Generates a narrative including vision agentic enhancements.
    """
    # Generate the dynamic narrative
    story = preparePromptForNarrative(analysis, png_files, interactive_files, api_proxy_token, api_proxy_url)

    # Analyze visualizations using vision models (placeholder)
    vision_insights = analyzeVisualizations(png_files, api_proxy_token, api_proxy_url)
...
def analyzeAndGenerateOp(file_path, api_proxy_token, api_proxy_url):
    """
    ...
    df, analysis = performPrimitiveOperations(file_path)
    png_files = visualizeAndSaveCharts(df, output_dir)
    interactive_files = generateInteractiveVisualizations(df, output_dir)
    story = narrateStoryWithVisuals(analysis, png_files, interactive_files, api_proxy_token, api_proxy_url)
...
def main():
    """
    Main function to process passed CSV file.
    """
    if len(sys.argv) != 2:
    ...
    api_proxy_token, api_proxy_url = getApiMetadata()

    if os.path.exists(file_path):
        print(f"Processing file: {file_path}")
        logging.info(f"Processing file: {file_path}")
        output_dir = analyzeAndGenerateOp(file_path, api_proxy_token, api_proxy_url)
